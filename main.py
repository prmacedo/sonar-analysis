...
# run_command("docker-compose run --rm -v /home/paulo/Projetos/observatorio-frontend:/usr/src/obi-frontend sonar-scanner-cli \
#   -Dsonar.projectKey=obi \
#   -Dsonar.projectName=obi \
#   -Dsonar.sources=obi-frontend \
#   -Dsonar.host.url=http://172.17.0.1:9000 \
#   -Dsonar.login=admin \
#   -Dsonar.password=admin \
#   -Dsonar.exclusions=**/*.java > sonar_analysis.log 2>&1")
...
def wait_for_sonarqube():
  ...
  while True:
    try:
      ...
      print(f"Aguardando o SonarQube ficar disponível... (status: {status})")
...
def collect_metrics(token):
  ...
  if response.status_code == 200:
    ...
    print(f"({response.status_code}) Erro ao coletar métricas: {response.text}")
    exit(1)

def save_to_csv(metrics):
  print("Salvando métricas em CSV...")
  filepath = os.path.join(OUTPUT_PATH, 'sonar_analysis.csv')
